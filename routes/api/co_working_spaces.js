...
router.put('/:id',(req,res)=>{
  ...
  Co_working_space.findOneAndUpdate({_id :todoID},{$set :{BasicInfo: req.body.BasicInfo}},{new :true},(err,result)=>{})
...
router.delete('/:id', async (req,res) => {
    try {
     ...
     res.json({msg:'Co_Working_space was deleted successfully'})
    }
    catch(error) {
        // We will be handling the error later
        console.log(error)
...
router.post('/Business_Plans_Offered/:id', async (req, res) => {
	const todoID = req.params.id;
          ...
          Co_working_space.findOneAndUpdate({_id :todoID},{$push: {Business_Plans_Offered:req.body.Business_Plan_Offered}},{new :true},(err,result)=>{res.json({data:result})})
			}
...
router.put('/Business_Plans_Offered/:id',async (req, res) => {
  ...
  Co_working_space.findOneAndUpdate({_id :co_Working_spaceid},{$set :{Business_Plans_Offered: result}},{new :true},(err,result)=>{res.json({data:result})})
}})
...
router.post('/Facilites/:id', async (req, res) => {
	const todoID = req.params.id;
          ...
          Co_working_space.findOneAndUpdate({_id :todoID},{$push: {Facilites:req.body.Facility}},{new :true},(err,result)=>{res.json({data:result})});
			}
...
router.delete('/Facilites/:id',async (req, res) => {
	...
	Co_working_space.findOneAndUpdate({_id :co_Working_spaceid},{$set :{Facilites: result}},{new :true},(err,result)=>{res.json({data:result})})
}})
// update Facilites
