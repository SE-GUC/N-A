...
router.post('/Facilites/:id', async (req, res) => {
	const todoID = req.params.id;
          ...
          Co_Working_space.findOneAndUpdate({_id :todoID},{$push: {Facilites:req.body.Business_Plan_Offered}},{new :true},(err,result)=>{})
				  res.json({msg:'done'});
			}
...
router.delete('/Facilites/:id',async (req, res) => {
	...
	for(var i=0;i<(X.Facilites).length;i++){
		if((X.Rooms)[i]!=req.body.Business_Plan_Offered)
			result.push((X.Facilites)[i])
	}
	Co_Working_space.findOneAndUpdate({_id :Co_Working_spaceid},{$set :{Rooms: result}},{new :true},(err,result)=>{})
